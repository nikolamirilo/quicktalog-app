...
-- PATCHED COPY (pglite-harness/rls.patched.sql): executed on PGlite 0.3.16 (PostgreSQL 17.5) against a Supabase stub.
--   Every change versus the draft rls.sql is marked "-- PATCH Pn" and explained in ../pglite-results.md:
--   P1 (04, 06) enable RLS on all 12 tables in 04 with temporary legacy anon policies (closes the Phase 1/2 window)
--   P2 (01)     check ADMIN OPTION before the GRANT fallback and fail with an actionable message
--   P3 (01, 03) private schema and private tables are explicitly owned by postgres whoever applies the migration
--   P4 (07)     app_rls login search_path = public, pg_temp (temp-table shadowing backstop)
--   P5 (08)     imported users are recognised by a pre-claimed migration.clerk_user_map row, because GoTrue
--               admin.createUser INSERTs auth.users first and applies app_metadata in a later UPDATE
--               (supabase/auth internal/api/admin.go adminUserCreate: tx.Create(user) ... user.UpdateAppMetaData)
...
begin
  foreach r in array array['app_user', 'app_public'] loop
    ...
    if not pg_catalog.pg_has_role('postgres', r, 'SET') then
      -- PATCH P2: GRANT needs ADMIN OPTION (PG16+). PGlite run: without it the GRANT fails with a bare 42501
      -- "permission denied to grant role". Fail with the fix instead.
...
alter schema private owner to postgres;   -- PATCH P3: if a superuser applies this, asAdmin (postgres) must still own private
...
-- six tables that no anon path uses. PATCH P1: RLS is now enabled on ALL twelve tables here (see 4.8) and the
-- paragraph below describes the ORIGINAL plan, which left a cross-tenant window. users/catalogues/analytics/newsletter/subscriptions/job_logs get RLS in
...
-- 4.8 PATCH P1: RLS on the other six tables NOW, so the app_user/app_public policies above are enforced the
-- moment the first withUser/withPublic call site ships (PGlite run on the original: with RLS off, a converted
-- withUser block read/deleted other owners' catalogues, renamed other users and read their subscribers).
-- Today's anon supabase-js paths keep exactly their current reach through temporary permissive policies that
-- mirror the current anon GRANTs (20260912093000_lockdown_privileges_and_schema_fixes.sql:57-100). postgres
-- (Drizzle today) and service_role (worker) bypass RLS. Migration 06 drops these policies with the grants.
...
create policy legacy_anon_users         on public.users         for all to anon using (true) with check (true);
create policy legacy_anon_catalogues    on public.catalogues    for all to anon using (true) with check (true);
create policy legacy_anon_subscriptions on public.subscriptions for all to anon using (true) with check (true);
create policy legacy_anon_analytics     on public.analytics     for all to anon using (true) with check (true);
create policy legacy_anon_newsletter    on public.newsletter    for select to anon using (true);
create policy legacy_anon_job_logs      on public.job_logs      for insert to anon with check (true);
...
alter table public.users         enable row level security;   -- no-op after PATCH P1 (already on since 04)
alter table public.catalogues    enable row level security;
alter table public.analytics     enable row level security;
alter table public.newsletter    enable row level security;
alter table public.subscriptions enable row level security;
alter table public.job_logs      enable row level security;
-- PATCH P1: remove the temporary legacy anon policies together with the anon grants below.
drop policy if exists legacy_anon_users         on public.users;
drop policy if exists legacy_anon_catalogues    on public.catalogues;
drop policy if exists legacy_anon_subscriptions on public.subscriptions;
drop policy if exists legacy_anon_analytics     on public.analytics;
drop policy if exists legacy_anon_newsletter    on public.newsletter;
drop policy if exists legacy_anon_job_logs      on public.job_logs;
...
-- 8.1b PATCH P5: the Clerk id map must exist before the sign-up trigger (moved here from RUNBOOK R1; R1 stays
-- idempotent). The import script inserts (clerk_user_id, supabase_user_id, 'pending') BEFORE admin.createUser.
...
begin
  ...
  -- PATCH P5: at AFTER INSERT time GoTrue has not yet written app_metadata for admin.createUser, so the check above
  -- never matches an import (PGlite run: every imported user got a default-plan users row and the RUNBOOK then
  -- aborted with 'a target uuid is already present in public.users'). The pre-claimed map row is visible.
