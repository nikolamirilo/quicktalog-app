...
export async function revalidateCatalogueByTag(catalogueName: string) {
  try {
    ...
    console.error(`Failed to revalidate tag catalogue-${catalogueName}:`, error)
    return { success: false, error: error.message }
  }
}
...
export async function revalidateAfterCatalogueChange(catalogueName?: string) {
  try {
    ...
    if (catalogueName) {
      revalidations.push(revalidatePath(`/catalogues/${catalogueName}`, "page"))
      revalidations.push(Promise.resolve(revalidateTag(`catalogue-${catalogueName}`)))
    }
