...
export class Sql {
	...
	/**
	 * The SQL for a phase key. Migrations that live in the repo are read from
	 * `supabase/migrations`, so this harness always tests the files that will
	 * actually be pushed. Everything else (operator scripts, rollbacks and
	 * migrations not written yet) comes from the plan extracts in `dir`.
	 */
	text(key) {
		const migration = phaseMigrationPath(key);
		if (migration) return fs.readFileSync(migration, "utf8");
