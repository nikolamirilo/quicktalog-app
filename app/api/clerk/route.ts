...
// Constants
const HANDLED_EVENT_TYPES = ["user.created", "user.updated", "user.deleted"] as const
...
function extractUserEmail(emailAddresses?: Array<{ email_address: string }>): string | null {
  if (!Array.isArray(emailAddresses) || emailAddresses.length === 0) {
    return null
  }

  const primaryEmail = emailAddresses[0]?.email_address
  return typeof primaryEmail === "string" ? primaryEmail.trim().toLowerCase() : null
}
...
function buildUserData(event: ClerkWebhookEvent): UserData | null {
  ...
  const baseUserData: UserData = {
    ...
    cookie_preferences: public_metadata?.cookieConsent || defaultCookiePreferences,
  }
...
async function handleUserUpsert(supabase: any, userData: UserData): Promise<void> {
...
async function handleUserDeletion(supabase: any, userId: string): Promise<void> {
  // Validate userId
...
async function sendWelcomeEmailSafely(email: string | null, name: string): Promise<void> {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error("Webhook verification timeout")), 10000)
    )

    event = (await Promise.race([verificationPromise, timeoutPromise])) as ClerkWebhookEvent
    ...
    switch (event.type) {
      ...
      case "user.updated": {
        ...
        if (event.type === "user.created") {
          ...
          sendWelcomeEmailSafely(userData.email, userData.name).catch((error) => {
      ...
      case "user.deleted": {
        ...
        const processingTime = Date.now() - startTime
        console.log(`Successfully deleted user ${userId} in ${processingTime}ms`)
    ...
    if (errorMessage.includes("verification") || errorMessage.includes("timeout")) {
