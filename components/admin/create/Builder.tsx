...
function Builder({ type, initialData, onSuccess, userData }: BuilderProps) {
  ...
  const isStepValid = (step: number): boolean => {
    ...
    if (step === 2) {
      ...
      const hasErrors = Object.keys(errors).length > 0
      return formData.services.every((category) => !!category.name.trim()) && !hasErrors
    }
    if (step === 3) {
      const hasErrors = Object.keys(errors).length > 0
      return formData.services.every((category) => category.items.length > 0) && !hasErrors
    }
    if (step === 4) {
      const hasErrors = Object.keys(errors).length > 0
      return !hasErrors
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const catalogueUrl = responseData.catalogueUrl || `/catalogues/${finalSlug}`
